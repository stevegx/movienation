...
export default function MoviesMenuList() {

    const location = useLocation()
    const moviePage = location.state[0].movieProvider.splice(0,100)
  ...
  return (
    <div className='flex bg-neutral-900 text-zinc-50'>
        ...
        <div className='w-screen h-screen overflow-x-hidden scroll-smooth'>
            <h1 className='text-3xl font-bold text-zinc-50 cursor-pointer hover:brightness-125 active:brightness-90 text-center mt-3'> <span className='text-blue-600  font-black'>Movie</span>Nation</h1>
              ...
              {moviePage.map(movie =>{
                          return(
                              <div onClick={()=>{navigate(`/movies/${movie.title}`,{state:{movie}})}}
                                  ...
                                  <img className='rounded-xl w-1/12 max-md:w-1/4' src={image+movie.poster_path} alt={movie.title} />
                                  <div className='flex flex-wrap gap-5'>
                                    <h1>{movie.title}</h1>
                                    <h1 className='text-gray-300 group-active:text-blue-500'>({movie.release_date})</h1>
                                    <div className='flex'>
